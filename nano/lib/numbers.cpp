...
bool nano::public_key::decode_account (std::string const & source_a)
{
	...
	if (!error)
	{
		...
		auto kti_prefix (source_a[0] == 'k' && source_a[1] == 't' && source_a[2] == 'i' && (source_a[3] == '_' || source_a[3] == '-'));
		...
		error = (xrb_prefix && source_a.size () != 64) || (kti_prefix && source_a.size () != 64) || (nano_prefix && source_a.size () != 65);
		if (!error)
		{
			if (xrb_prefix || kti_prefix || nano_prefix || node_id_prefix)
			{
				auto i (source_a.begin () + ((xrb_prefix || kti_prefix) ? 4 : 5));
				if (*i == '1' || *i == '3')
				{
